            ...
            if (exists.length == 0 && (date2 == today)) {
                ...
                if (content) {
                    ...
                    /*if (src != "") {
                     ...
                     var img_url = src;
                     console.log(img_url)
                     utils.downloadSync(img_url, dir, path);
                     //定制尺寸缩略图
...
function dede_archives_find(title) {
    var archives = utils.query("select * from dede_archives where title= ?", title);
    return archives;
}

function dede_archives_findOne() {
    var archives = utils.query("select id from dede_archives order by id desc limit 1");
    return archives;
}
...
function dede_archives(obj) {
    ...
    var archives = utils.query("insert into dede_archives set ?", obj);
    return archives;
}
...
function dede_arctiny(obj) {
    ...
    var uploads = utils.query("insert into dede_arctiny set ?", obj);
...
function dede_addonarticle(obj) {
    //(id,typeid,typeid2,arcrank,channel,senddate,sortrank,mid) values (?,?,?,?,?,?,?,?)
    var uploads = utils.query("insert into dede_addonarticle set ?", obj);
